...
class RegularTextView(context: Context?, attrs: AttributeSet?) : AppCompatTextView(context, attrs) {
    ...
    init {
        if (attrs != null) {
            try {
                var mTypeFace = MyApp.getTypeFaceCache().get("QuickSand-Regular")
                if (mTypeFace == null) {
                    mTypeFace = Typeface.createFromAsset(getContext().assets,
                            String.format("fonts/%s.ttf", "QuickSand-Regular"))

                    // Cache the typeface
                    MyApp.getTypeFaceCache().put("QuickSand-Regular", mTypeFace)
                }
                typeface = mTypeFace
